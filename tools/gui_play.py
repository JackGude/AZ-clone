...
# Colors
COLOR_LIGHT_SQ = (140, 185, 185)
COLOR_DARK_SQ = (50, 75, 100)
COLOR_BORDER = (40, 40, 40)
COLOR_PANEL = (50, 50, 50)
COLOR_TEXT = (220, 220, 220)
COLOR_HIGHLIGHT = (255, 227, 169, 150)
COLOR_VALID_MOVE = (114, 92, 173, 150)
...
class ChessGUI:
    def __init__(self, net, encoder, time_limit, device):
        ...
        self.status_font = pygame.font.SysFont(
            "sans-serif", FONT_SIZE_STATUS, bold=True
        )
        ...
        self.new_game_button = pygame.Rect(
            BOARD_SIZE + BORDER_SIZE + 30, SCREEN_HEIGHT - 80, 200, 50
        )
        self.unicode_map = {
    ...
    def _draw_board_and_pieces(self):
        ...
        for r in range(8):
            for f in range(8):
                ...
                rect = pygame.Rect(
                    f * SQUARE_SIZE, r * SQUARE_SIZE, SQUARE_SIZE, SQUARE_SIZE
                )
    ...
    def _draw_coordinates(self):
        ...
        for i in range(8):
            ...
            self.screen.blit(
                label,
                (
                    BORDER_SIZE + i * SQUARE_SIZE + (SQUARE_SIZE // 2 - 5),
                    BOARD_SIZE + 5,
                ),
            )
    ...
    def _draw_panel(self):
        ...
        for i, move in enumerate(
            reversed(self.env.board.move_stack[-max_moves_to_show:])
        ):
    ...
    def _handle_click(self, pos):
        ...
        if self.selected_square is not None:
            move_to_make = next(
                (m for m in self.valid_moves if m.to_square == clicked_square), None
            )
            if move_to_make:
                # Handle pawn promotions (auto-promote to queen for simplicity)
                if move_to_make.promotion and chess.square_rank(
                    move_to_make.to_square
                ) in [0, 7]:
        ...
        if (
            piece and piece.color == self.env.board.turn
        ):  # Can only select pieces of the current player
            self.selected_square = clicked_square
            self.valid_moves = [
                m for m in self.env.board.legal_moves if m.from_square == clicked_square
            ]
    ...
    def run(self, human_color):
        ...
        while running:
            is_human_turn = (
                self.env.board.turn == self.human_color
            ...
            if not is_human_turn and not self.game_over and not self.ai_is_thinking:
                ...
                self.ai_thread = threading.Thread(
                    target=self._run_ai_search, daemon=True
                )
            ...
            if self.ai_is_thinking and self.ai_thread and not self.ai_thread.is_alive():
                ...
                self.last_q_value = (
                    q_value if turn_before_move == chess.WHITE else -q_value
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Play a graphical game of chess against the AlphaZero model."
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
